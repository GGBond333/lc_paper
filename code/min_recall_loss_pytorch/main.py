...
def run_experiment(model_type='mlstm_fcn', loss_type='lse', alpha=10.0, epochs=50, batch_size=32):
    ...
    # 选择损失函数
    if loss_type == 'mean':
        criterion = MeanRecallLoss()
    elif loss_type == 'product':
        criterion = ProductRecallLoss()
    elif loss_type == 'softmax':
        criterion = SoftmaxRecallLoss(alpha=alpha)
    elif loss_type == 'lse':
        criterion = LSERecallLoss(alpha=alpha)
    elif loss_type == 'p_norm':
        criterion = PNormRecallLoss(alpha=alpha)
    else:
        # 默认使用交叉熵损失
    ...
    # 评估模型
    accuracy, min_recall, recalls = evaluate_model(model, test_loader, device)
